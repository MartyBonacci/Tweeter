...
import type { LoginData, RegisterData } from '~/models/auth/auth.validator';
...
export async function loginUser(data: LoginData): Promise<{ user: AuthUser; tokens: AuthTokens }> {
...
export async function registerUser(data: RegisterData): Promise<{ user: AuthUser }> {
...
export function generateAuthCookies(accessToken: string, refreshToken: string): string[] {
  return [
...
export const Index = {
  ...
  generateAuthCookies: generateAuthCookies,
};